# 人体仿真指南历史快照（2026-09-24 需求整理前）

> 归档：保留早期实现与验收语境，旧资产、关节数、GPU、坐标和接触结论可能已被后续证据取代。
> 当前使用说明见 [人体仿真指南](../human-simulation.md)，当前任务见 [计划](../../task_plan.md)。

## 键盘交互（2026-09-24）

`~/isaacsim/python.sh scripts/humans/keyboard.py` 已接入 W/S、A/D、空格、R、Esc，
显示实时物理姿态；按键、停止、复位与挡墙已有 GUI/物理实测。
仍为外力辅助控制，存在滑步及皮肤穿透；完整说明与截图见 [键盘控制](../keyboard-control.md)。
下文“实时皮肤仍静态”的结论仅适用于旧 `simulate.py` 路径，新键盘入口已解决。

## 当前实测状态（2026-09-24）

以 [AMASS 物理审计](../amass-physics-audit-2026-09-24.md) 为最新依据；下方旧日期的
资产、坐标转换、接触归因和验收结论保留作历史记录，不代表当前状态。
本机实际有 2198 条 AMASS；SMPL neutral 已用于实际物理姿态的蒙皮。
多轴 rig 为 57 DOF / 62 连杆 / 22 碰撞体。
完整侧向行走与后退行走在有限外力辅助下通过关节与根轨迹门槛，均非自主平衡。
无辅助侧向行走、快速拳击尚未通过；坐地起立与蹲行仍需修复控制/限位/接触。
地板和挡墙有实际 PhysX 接触证据，仍有毫米级地面和厘米级墙面皮肤重叠。

可复现完整侧行（根辅助力最大 1500 N、力矩最大 400 Nm，配置与实测写入产物）：
...
将 `--headless` 改成 `--gui --hold-seconds -1` 可在计算完成后查看实际记录回放。
计算期间的旧静态皮肤不作为动作证据；`render_recording.py` 可独立渲染记录帧。
`view_amass.py` 是运动学参考预览，不能证明碰撞响应或自主运动完成。
...
串联转动关节，中间插入零长度 proxy 连杆）。单轴配置 proxy 数量为 0；
`configs/humans/human_smpl_multiaxis.yaml` 把 AMASS 实际会转动的每个关节配成 3 轴
（19 关节 / 57 DOF / 62 连杆）。依据是实测而非推断：单轴 rig 只能表达
**0.0000%** 的 AMASS 帧，两轴仍有 1.6–12° 中位偏差，只有三轴能覆盖整个转动群。

链中**第一个** proxy 挂在关节自己的 SMPL 父节点上，因此只有它必须承载骨向量偏移，
其余 proxy 与实连杆同点。这一条曾被写反（所有 proxy 被强制 `localPos0=0`），
结果整具人体被压缩到根节点、1.70 m 身高量成 0.803 m，而连杆数、质量分配和
「plan 构建成功」类断言全部照常通过。缺陷、根因与修复证据见
[`docs/amass-replay-defect.md`](../amass-replay-defect.md)。
