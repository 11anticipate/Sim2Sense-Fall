# 项目计划历史快照（2026-09-24 需求整理前）

> 归档：保留整理前的阶段、失败、修复和证据，不作为当前待办清单。
> 本文的“当前”“未取得”“正在运行”等均为原记录用语；当时运行已结束。
> 当前计划见 [task_plan.md](../../task_plan.md)，证据导航见 [文档索引](../README.md)。

## 2026-09-24 键盘交互与实时物理显示

- [x] 检查 AMASS 可循环步态、键盘事件、物理时钟和显示更新接口。
- [x] 实现配置驱动的移动/转向/停止与有限外力辅助，保留碰撞响应。
- [x] CPU 测试后，在 Isaac 验证按键流程、实时蒙皮、停止及挡墙，保存截图和状态。
- [x] 更新运行说明、实测边界与后续自主平衡任务。

键盘控制不等于自主平衡；本轮先把已有辅助物理控制变成可交互工具。
研究记录：`docs/keyboard-control.md`。
已通过 CPU 键盘/限速/防目标积累/接触点速度测试。GUI 10.3 s、1236 物理步与回调一致，
实时皮肤 + R 复位无张量失效。检测到滑步和皮肤穿地，仍非完整动作质量验收通过。
最终 GUI 最大关节误差 13.787°；脚底滑动 p95=0.751 m/s、皮肤穿地 9.86 mm 未通过。
键盘挡墙：根被阻挡、2836 接触点、目标偏离最多 0.10 m；皮肤局部穿墙约 51.2 mm。
CPU 281 passed / 9 skipped。启动、按键、截图与质量边界见 `docs/keyboard-control.md`。

## 2026-09-24 AMASS 动作与物理交互实证复核

- [x] 阅读实际导入、控制、接触、蒙皮代码及失败日志；本机 2198 条 AMASS 存在。
- [x] GPU 权限阻塞解除，RTX 4060 / driver 595.91.07 可见。
- [x] 修正 AMASS 模型局部坐标与 Z-up 世界坐标混用，并独立验证原始动作几何。
- [x] 复跑 CPU 和 Isaac 基线；用实际状态、截图检查动作、接触和穿透。
- [x] 修复实测确认的问题，记录自由运动与辅助运动的真实边界。
- [x] 更新进度及研究笔记，列出动作控制仍缺的能力与可复现命令。

用户要求：必须查看截图或等效实际状态证据，不能根据文档或单纯测试通过宣称动作完成。
当前调查笔记：`docs/amass-physics-audit-2026-09-24.md`。
已实测发现并修复：接触句柄可解码、漏掉 pelvis collider、无 head/hand 末端碰撞体、
PD 缺少参考速度、AMASS 筛选未检查关节限位。新增有限根力辅助（明确非自主平衡）。
完整侧行 7.275 s、后退 7.867 s 在有限根力辅助下通过跟踪；自主行走仍失败。
挡墙负对照确实阻止人体继续走，但有约 25 mm 皮肤重叠。不能标记阶段 7 完成。
最终人体 Isaac 基线通过（22 碰撞体、最大 PD 误差 11.462°、重力下降 1.0176 m）。
CPU 回归 277 passed / 9 skipped；截图和剩余能力详见本轮审计。

## 2026-09-23 本轮证据复审与剩余任务

- [x] A：复核 d78b34c / 6084df8、已有产物、相机/出生点/网格/标签/信道计算。
- [x] B：移除自由根稳定期写入，新增5秒静态站立配置并通过；后推物理跌倒通过，自然行走未做。
- [x] C：补人体电磁参数来源，实现公寓到 Sionna，修复信道采样与元数据。
- [x] D：CPU 246 passed / 8 skipped，bundled USD 9 passed；场景/人体Isaac及GPU公寓物理人体CIR通过。

起点 HEAD=6084df8；已有未提交 docs/physics-interaction-audit.md 修改需保留。
本轮权威状态与命令：docs/verification-2026-09-23.md；其后旧日期段落保留作历史记录。
当前已跑通：固定公寓 + SMPL有限PD静态站立/后推物理跌倒 → 完整公寓Sionna复数CIR。
静态站立使用 configs/humans/human_smpl_stable.yaml（显式平足碰撞近似、有限强PD），旧neutral基线不自动替换。
真实AMASS本机已存在2198条；去重切片2146条。3条CMU"导入通过"仅指读取无报错，其筛选结论是 accepted=False。
2026-09-23 已修 AMASS 整机基（偏航）缺陷并让批量筛查逐文件容错：抽样 120 条得 28 条跌倒候选，
但 0 条可被当前单轴 rig 表达，卡点全在肘/肩；详见 docs/progress.md 同日条目。
剩余：视口姿态显示（stage 皮肤只在建模时写一次，逐帧写会作废 PhysX tensor 视图，已回滚需另找路径）、
clip→DOF 多轴分解（planner 已支持多轴链，映射器尚未分解）、自然行走/平衡恢复、
多方向有效跌倒与网格穿地修复、动态家具同步、人体EM校准、50 Hz数据集及检测模型。
2026-09-23 已修手臂 T-pose：肩轴 y→x、肘 y→z，中性姿态由 visualization.default_pose_rad 声明一次
并经 rig.apply_neutral_pose 注入参考；站立横向半跨度 91.3→22.3 cm，两条 Isaac 试验仍全通过。
2026-09-23 已关掉 simulate.py / verify.py 对 tensor 接触视图的请求（该视图在本机构建不出来，
请它就是每次运行 4,368 行 [Error]，外加 SimulationManager 事件回调里脚本接不住的
AttributeError，时间线停止时再触发一次，即 GUI 按中断看到的那一屏）。改为不请求后真机
正负对照 4,368→0 行 [Error]，接触通道（physx 轮询 + 几何归因）与 601 帧接触历史不变；
`HumanRuntime` 新增 `contact_tensor_view_reason` 区分「没请求/请求了但坏了/无可请求对象」。
详见 docs/progress.md 同日条目与 docs/physics-interaction-audit.md RC-1。
...
- [ ] 阶段 7：SMPL/Isaac分层验收、5秒自由静态站立、后推物理跌倒与同步网格已通过；自然行走/恢复和多方向有效跌倒仍未完成。
      （2026-09-23：SMPL资产已加载；AMASS真实CMU/Transitions共2198条已在本机，3条CMU导入复验通过。）
- [x] 阶段 8：首个固定公寓 + 实际物理后推跌倒 → 复数CIR端到端smoke通过；尚非训练数据集。
...
- 2026-09-21：**已解除** — 早前工作树 `.git` 是只读空目录；现在 `.git` 可写，存在 `main`
  ...
  `feature/indoor-scene` 分支，`main` 保持不动。详见 [`docs/progress.md`](../progress.md)。
...
- [x] 修掉 SMPL 导入被偏航 90° 的真缺陷（文件帧 `X=横/Y=上/Z=前` vs 管线帧
      ...
      详见 [`docs/mesh-orientation-defect.md`](../mesh-orientation-defect.md)。
...
- [x] simulate.py 已采集5秒站立及后推 physics_trial，见 artifacts/review_20260923/final_physics；多方向仍未通过。
- [x] --write-slice 对真实2198条目录运行；2146链接/52重复stem，3条CMU导入通过，未做全量筛选。
...
当前验收器 10 项检查通过（CPU 回退），失败负例实际返回非零；详见 `docs/indoor-scene-remediation.md`。构建与 GUI 查看指令见 [`docs/indoor-scene.md`](../indoor-scene.md)。

**2026-09-22 复审修复状态：阶段 7 的 CPU/USD/Isaac 分层验收已通过。** 资产审计已纠正：项目内存在并已加载 SMPL v1.1.0 neutral/male/female 三个模型，neutral 的 6890 顶点、13776 面、300 shape directions 和静止蒙皮复验通过；AMASS 原始序列仍未取得。PD 验收已隔离人体碰撞并重置传送后的速度，最新 headless Isaac 运行通过，14 个 DOF 最大跟踪误差 1.540°（容限 15°），驱动关闭负对照、重力回落、地面穿透和场景哈希检查均通过。历史问题与证据仍见 [`docs/stage7-review.md`](../stage7-review.md)。固定室内场景中的 CPU/USD 数据链路
...
[`docs/mesh-export.md`](../mesh-export.md)。
...
- [ ] 筛选少量 AMASS 短动作，记录来源子集、原始序列、人物、模型表示、帧率和许可；不预设存在足够的跌倒序列或可靠动作标签。**导入器、来源哈希、Y-up→Z-up、SMPL-H→SMPL 重定向和候选筛选已完成；本机 `data/humans/amass_raw` 已有 2198 条已授权序列（CMU + Transitions）。**
      **2026-09-24 卡点已解**：单轴 rig 无法表达任何真实 AMASS 帧（实测 0.0000%），
      两轴仍有 1.6–12° 中位偏差；缺的确实是 clip→DOF 多轴分解，不是数据。
      已实现 `rotations.split_rotation`（1/2/3 轴精确分解 + 测地残差）、
      `rig` 按骨架关节分组分解、`configs/humans/human_smpl_multiaxis.yaml`
      （19 关节 × 3 轴 = 57 DOF / 62 连杆）。同一批抽样现在 **100.0000% 可表达、0 个角度被限位裁剪**。
      依据与复现命令：`scripts/humans/measure_axes.py`；缺陷全记录见
      [`docs/amass-replay-defect.md`](../amass-replay-defect.md)。剩余：真实动作的物理回放与跌倒标签。
...
- [x] 2026-09-23核实本机 amass_raw/CMU 已有2088条npz；小批导入通过，原下载未完成记录已过时。
...
## 2026-09-24 AMASS 物理试验打通（时序 + 锚定双缺陷修复）

- [x] **实测并修复 `app.update()` 时序缺陷（Fix B）**：本 build 一次 update 固定推进
      1/60 s 物理时间（dt=1/120 时 2 步/update；loop-isaac 与 runLoops 手动模式设置均
      无效），所有「秒→update 数」窗口实际跑了 2 倍物理时间——这是 −5.0 m pelvis 的
      时序侧根因。修复：`common.step_physics`（SimulationManager.step + 步数读回断言）
      替换 verify/simulate 的全部秒数窗口；`GRAVITY_SETTLE_SECONDS` 3.0→6.0 保真换算。
- [x] **实测并修复 AMASS 根运动锚定缺陷（Fix A）**：`simulate.py` 曾把采集坐标系
      `root_translation[0]` 当公寓世界位姿（clip 10_05 pinned xy=(11.211,−0.683) 在
      VOID → 身体坠空）。修复：筛选前先 `normalize_root_motion`（与 view_amass 一致）。
- [x] **verify 复验全绿**：37 PASS / 0 FAIL（重力回落、末段蠕动 0.085 mm、PD 11.5°<15°、
      穿地 −0.0000 m）。
- [x] **CPU 筛选**：57 DOF 多轴 rig 下 12/12 真实 AMASS 可表达（残差 0.000 rad），
      3 条 accepted 跌倒候选（135_02 / 17_01 / 22_03）。
- [ ] **真实 AMASS 物理试验（多轴 rig，含接触与标签）**：`trials_amass_anchored`
      正在运行/结果待记录。
- CPU 门禁：pytest 269 passed / 9 skipped；ruff、compileall 通过。
