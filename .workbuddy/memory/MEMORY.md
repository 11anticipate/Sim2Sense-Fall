...
## 「看到动作」≠「物理站得住」（2026-09-23）

- **P0-3 不阻塞「在 Isaac Sim 里看动作」。** 查看器（`view_amass.py`）是 **kinematic
  replay**：每帧直接写 DOF + 根位姿并清零速度，不跑控制环。P0-3 管的是 `simulate.py`
  自由根试验里人体能否靠 PD 自己站住（现在稳定期每步 `set_root_pose` 传送骨盆撑着）。
- `view_amass.py` 的 `--amass-root` 已改为**可选**，省略就读内置
  `configs/humans/motions.yaml`。两种来源返回同一个 `MotionClip`，回放循环只有一份。
  `--fall-only` 判据随来源变（AMASS 筛选器 / `fall_reference` 标签）。
- **`scripts/humans/verify.py` 的重力回落试验里，人体是「被举高后自由倾倒」**：它没有
  驱动（`set_control_scale(0.0)`），所以必然会倒并在地上滑一段。判据用
  `DIVERGENCE_HEIGHT_FACTOR = 2.0` × 站立骨盆高度 + 「末段是否静止」，**不是**固定 1.0 m。
- **人体出生点离墙只有 0.52 m**（`scene_spawn_point` 选的是「离家具最远」的点，
  结果落在客厅 (9.320, 0.520)，客厅 bounds x 8.800..18.480 / y 0.000..6.600，
  −X 与 −Y 墙各 0.52 m）。这正是审计里 **P2-1「出生点避开墙」**未做。
  它会污染需要空间的试验：人体向前(+X)倒有 9.16 m 空间，但若倒向 ±Y 会在 0.5 m 处撞墙。
- 判定「人体倒向哪边」要用**实测轨迹**：修复后实测
  `direction = -2.0 deg from +X`、水平位移 1.3855 m、末态 z = 0.3029 m（躺平后不再动）。
  即人体朝**管线的前向 +X** 倒 —— 这本身就是帧修复正确的又一个独立佐证。
