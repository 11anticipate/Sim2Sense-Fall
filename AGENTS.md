...
```bash
python3 scripts/build_indoor_scene.py --dry-run                 # CPU，无需 Isaac Sim
~/isaacsim/python.sh scripts/build_indoor_scene.py --headless   # 导出 USD
~/isaacsim/python.sh scripts/verify_indoor_scene.py             # 场景与物理 smoke test
```

注意：独立运行的 Isaac Sim 默认不会把 PhysX 挂到 USD stage 上，物理步数在涨但物体不动。
需要物理时必须先调 `sim2sense_fall.isaac_scene.activate_physics()`；只断言「漂移为 0」
会把「物理没跑」误判成通过，因此 `verify_indoor_scene.py` 用「抬高后落回」作正向对照。
